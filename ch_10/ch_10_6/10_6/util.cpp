...
auto Util::repeats(const std::string_view s, char c, unsigned pos) -> void
{
...
auto Util::build_new_case(const std::map <std::string, int>& m
	, std::string_view rmn_code, int current_index, int new_index) -> std::map<std::string, int>::const_iterator
{
...
auto::Util::sum(const std::map<std::string, int>::const_iterator it1_new_case
...
auto Util::parse_rmn_by_placevalue(std::string& result, const std::map<std::string
	, int>& map, char rmn_letter, int& val, char next_rmn_letter) -> void
{
...
auto Util::format_string(std::string& target_string) -> void
{
	...
	target_string[0] = static_cast<char>(toupper(target_string[0]));

	std::transform(target_string.cbegin() + 1, target_string.cend(), target_string.begin() + 1
		, [](unsigned char letter) {return std::tolower(letter); });
}

auto Util::skip_input(std::istream& is, char terminator) -> void
{
